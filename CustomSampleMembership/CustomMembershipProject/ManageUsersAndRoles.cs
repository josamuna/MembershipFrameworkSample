...
namespace CustomMembershipProject
{
    public partial class ManageUsersAndRoles : Form
    {
        private CustomMembershipProvider userProvider = null;
        private CustomRoleProvider roleProvider = null;
        ...
        private void loadUsers()
        {
            ...
            //lstUsers.DataSource = valuesUser;
        }
        ...
        private void clearListBox2(ListBox list)
        {
        ...
        private void btnUnlockUser_Click(object sender, EventArgs e)
        {
            try
            {
                bool status = userProvider.UnlockUser(txtUsername.Text);
        ...
        private void btnResetUserPassword_Click(object sender, EventArgs e)
        {
            try
            {
                string newPassword = userProvider.ResetPassword(txtUsername.Text, txtPasswordAnswer.Text);
        ...
        private void btnChangeUserPassword_Click(object sender, EventArgs e)
        {
            try
            {
                bool status = userProvider.ChangePassword(txtUsername.Text, txtOldPassword.Text, txtNewPassword.Text);
        ...
        private void dgvUsers_SelectionChanged(object sender, EventArgs e)
        {
            try
            {
                if(dgvUsers.SelectedRows.Count > 0)
                {
                    ...
                    txtPassword.Text = userProvider.GetPassword(user.UserName);
                    txtOldPassword.Clear();
                    txtNewPassword.Clear();
                    ...
                    // ApplicationName
                    txtApplicationName.Text = userProvider.ApplicationName;
                }
            }
            catch (ArgumentException ex)
            {
        ...
        private void btnDeleteRole_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (result == DialogResult.Yes)
                {
                    bool status = roleProvider.DeleteRole(txtRoleName.Text, true);
        ...
        private void btnSaveUsersInRoles_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Make Save
                roleProvider.AddUsersToRoles(userTable, roleTable);
        ...
        private void btnFinduserInRole_Click(object sender, EventArgs e)
        {
            try
            {
                if (lstRoleName.Items.Count > 0)
                {
                    ...
                    {
                        string selectedRole = lstRoleName.SelectedItem.ToString();
                        string[] usersInRole = roleProvider.FindUsersInRole(selectedRole, txtUserInRoleFind.Text);
